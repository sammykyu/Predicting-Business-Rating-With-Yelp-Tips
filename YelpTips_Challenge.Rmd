...
The study first estimates the predictive rating and mean square errors (MSE) from a naive model for comparison. Then we train and validate a LASSO model using _unigrams_ (single word in each term) and _bigrams_ (two adjacent words in each term) to see which one works better.  
...
ggplot(data = subset(wf_ugm, freq>10000), aes(word, freq)) + geom_bar(stat="identity", fill="blue") + 
  ggtitle("Unigram Word Frequencies") + theme(axis.text.x=element_text(angle=45, hjust=1))
...
wf_bgm <- PlotWordCloud(dtm2, 75)
ggplot(data = subset(wf_bgm, freq>5000), aes(word, freq)) + geom_bar(stat="identity", fill="yellow") + 
  ggtitle("Bigram Word Frequencies") + theme(axis.text.x=element_text(angle=45, hjust=1))
...
Features in bigram model with non-zero coefficients:  
(___`r names(out_bgm$coef[out_bgm$coef!=0])[-1]`___)  
